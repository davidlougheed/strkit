...
def _unzip_gt(vals):
    return (vals[0][0], vals[1][0]), (parse_cis(vals[0][1]), parse_cis(vals[1][1]))
...
class ExpansionHunterCalculator(BaseCalculator, VCFCalculatorMixin):
    ...
    def calculate_contig(self, contig: str) -> Tuple[int, int, int, List[Tuple]]:
        ...
        for cv in cvf.fetch(contig):
            ...
            cs_reps = tuple(sorted(zip(cs["REPCN"], cs["REPCI"]), key=lambda x: x[0]))
            ms_reps = tuple(sorted(zip(ms["REPCN"], ms["REPCI"]), key=lambda x: x[0]))
            fs_reps = tuple(sorted(zip(fs["REPCN"], fs["REPCI"]), key=lambda x: x[0]))
